  ...
  return (
    <ul>
      <div
        ...
        } py-2 px-6 flex-col items-start rounded-b-sm border border-gray-300 shadow bg-white z-50 ${
          right ? "right-0" : ""
        }`}
      >
        {data.map((p) => (
          <li key={p.id} className="px-2 py-3">
            <Link
              href={`/${variant}/${p.slug}`}
              className="flex py-1 px-0 justify-center items-center self-stretch"
            >
